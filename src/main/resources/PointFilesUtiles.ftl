...
<h3><form action="/files/delite/${id}/${bucketName}/${filename}" method="post">
    <input style="color: red; background: #f2f2f2" type="submit" value="удалить">
</form></h3>
<h3><button style="color: red" id="назад" onclick="location.href='/files/info/${id}/${bucketName}'">назад</button></h3>
<script>
    function redirectToPage1(fileInfo) {
      window.location.href = '/files/rename/${id}/${bucketName}/${filename}';
    }
</script>
<script>
    function redirectToPage2(fileInfo) {
      window.location.href = '/files/replace/${id}/${bucketName}/${filename}';
    }
</script>
</body>
</html>